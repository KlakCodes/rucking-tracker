...
type RuckFormProps = {
  ...
  initialEntry?: RuckEntry;
  onCancel: () => void;
  onSubmit: (entry: RuckEntry) => void;
};

export function RuckForm({
  title,
  subtitle,
  submitButtonText,
  initialEntry,
  ...
  const [distance, setDistance] = useState(initialEntry ? String(initialEntry.distance) : "");
  const [distanceUnit, setDistanceUnit] = useState<DistanceUnit>(initialEntry?.distanceUnit ?? "miles");
  const [durationHours, setDurationHours] = useState(
    initialEntry ? String(initialEntry.durationHours) : "",
  );
  const [durationMinutes, setDurationMinutes] = useState(
    initialEntry ? String(initialEntry.durationMinutes) : "",
  );
  const [ruckWeight, setRuckWeight] = useState(initialEntry ? String(initialEntry.ruckWeight) : "");
  const [weightUnit, setWeightUnit] = useState<WeightUnit>(initialEntry?.weightUnit ?? "lb");
